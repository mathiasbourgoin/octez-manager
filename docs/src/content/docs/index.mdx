---
title: Octez Manager
description: CLI and TUI for managing Tezos infrastructure
template: splash
hero:
  tagline: Deploy and manage Tezos nodes, bakers, and DAL infrastructure from your terminal.
  image:
    file: ../../assets/tui-demo.png
  actions:
    - text: Get Started
      link: /octez-manager/getting-started/introduction/
      icon: right-arrow
    - text: GitHub
      link: https://github.com/mathiasbourgoin/octez-manager
      icon: github
      variant: minimal
---

import { Card, CardGrid, Code, Tabs, TabItem } from '@astrojs/starlight/components';

## Install in seconds

<Tabs>
  <TabItem label="From source">
```bash
git clone https://github.com/mathiasbourgoin/octez-manager.git
cd octez-manager
opam install . --deps-only
dune build && dune install
```
  </TabItem>
</Tabs>

## Deploy a node in one command

```bash
octez-manager install-node \
  --instance mainnet \
  --network mainnet \
  --history-mode rolling \
  --bootstrap snapshot
```

Or use the interactive TUI:

```bash
octez-manager
```

---

## Why Octez Manager?

<CardGrid>
	<Card title="One tool, full stack" icon="rocket">
		Node, baker, accuser, DAL node, signer — all managed from a single interface.
	</Card>
	<Card title="Production ready" icon="setting">
		Systemd integration, proper user isolation, automatic snapshots.
	</Card>
	<Card title="Interactive & scriptable" icon="laptop">
		Beautiful TUI for operations, full CLI for automation.
	</Card>
	<Card title="Real-time monitoring" icon="document">
		Live logs, service status, sync progress — all in your terminal.
	</Card>
</CardGrid>

---

## Supported services

| Service | Command | Description |
|---------|---------|-------------|
| **Node** | `install-node` | Tezos L1 node with snapshot bootstrap |
| **Baker** | `install-baker` | Block production and attestations |
| **Accuser** | `install-accuser` | Double-baking detection |
| **DAL Node** | `install-dal-node` | Data Availability Layer |
| **Signer** | `install-signer` | Remote signing service |

---

## Manage instances

```bash
# List all instances
octez-manager list

# Control services
octez-manager instance mainnet start
octez-manager instance mainnet stop
octez-manager instance mainnet restart

# View logs
octez-manager instance mainnet logs

# Show details
octez-manager instance mainnet show
```
