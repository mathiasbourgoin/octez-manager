...
val ensure_system_directories :
  user:string -> group:string -> (unit, Rresult.R.msg) result

val validate_user_for_service : user:string -> (unit, Rresult.R.msg) result

val remove_service_account : name:string -> (unit, Rresult.R.msg) result
...
module For_tests : sig
  ...
  val with_overrides :
    ?is_root:(unit -> bool) ->
    ?run:(string list -> (unit, Rresult.R.msg) result) ->
