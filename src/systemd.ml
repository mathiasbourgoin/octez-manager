...
let node_prestart_script_body =
  ...
  ^ "          echo \"octez-manager prestart: snapshot source $URI not found\" \
     >&2\n" ^ "          return 1\n" ^ "        fi\n"
  ^ "        cp \"$URI\" \"$TMP\" ;;\n" ^ "    esac\n" ^ "    return 0\n"
  ^ "  fi\n\n" ^ "  NETWORK=\"${OCTEZ_SNAPSHOT_NETWORK_SLUG:-}\"\n"
  ^ "  KIND=\"${OCTEZ_SNAPSHOT_KIND:-}\"\n" ^ "  if [ -z \"$NETWORK\" ]; then\n"
  ^ "    echo \"octez-manager prestart: no network slug available for snapshot \
     download\" >&2\n" ^ "    return 1\n" ^ "  fi\n"
  ^ "  if [ -z \"$KIND\" ]; then\n"
  ^ "    KIND=\"${OCTEZ_HISTORY_MODE:-rolling}\"\n" ^ "  fi\n"
  ^ "  curl -fSL \"https://snapshots.tzinit.org/$NETWORK/$KIND\" -o \"$TMP\"\n"
  ^ "}\n\n" ^ "if fetch_snapshot; then\n"
  ^ "  EXTRA=\"${OCTEZ_SNAPSHOT_NO_CHECK:-0}\"\n"
  ^ "  if [ \"$EXTRA\" = \"1\" ]; then\n"
  ^ "    \"$NODE_BIN\" snapshot import --no-check --data-dir \"$DATA_DIR\" \
     \"$TMP\"\n" ^ "  else\n"
  ^ "    \"$NODE_BIN\" snapshot import --data-dir \"$DATA_DIR\" \"$TMP\"\n"
  ^ "  fi\n" ^ "else\n"
